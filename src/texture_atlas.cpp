...
bool AtlasManager::packTextures(unsigned char*& atlasData) {
    ...
    for (const auto& texture : m_pendingTextures) {
        ...
        region.topLeft = glm::vec2(
            posX / static_cast<float>(m_atlasWidth),
            posY / static_cast<float>(m_atlasHeight)
        );

        region.bottomRight = glm::vec2(
            (posX + texture->width) / static_cast<float>(m_atlasWidth),
            (posY + texture->height) / static_cast<float>(m_atlasHeight)
        );
